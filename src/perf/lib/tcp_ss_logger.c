...
static void fetch_ss_data(TCP_SS_LOGGER* logger) {
    ...
    while (fgets(line, sizeof(line), pipe)) {
        ...
        if ((strstr(line, "ESTAB") != NULL) || 
            ...
            snprintf(connection_line, sizeof(connection_line), "%s", line);
    ...
    )
{
    ...
    // 默认日志文件路径
    TcpSsLoggerSetLogFile(Logger, "/home/qwu26/msquic_cellular/artifacts/bbr_logs/tcp_ss_log.txt");
...
void TcpSsLoggerSetLogFile(_In_ TCP_SS_LOGGER* Logger, _In_ const char* FilePath) {
    ...
    Logger->LogFileHandle = fopen(Logger->LogFilePath, "w"); // 每次覆盖旧日志

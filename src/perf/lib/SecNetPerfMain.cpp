...
// Cellular ratio receiver for BBR optimization
extern "C" {
...
BOOLEAN EnableCellularRatio = FALSE;
...
QuicMainStart(
    ...
    // Initialize cellular ratio receiver
    // -cellular:0 = log only (record ratio, no algorithm change)
    // -cellular:1 = log + control (apply ratio-based gain)
    uint32_t CellularMode = UINT32_MAX;  // Not specified
    if (TryGetValue(argc, argv, "cellular", &CellularMode)) {
        WriteOutput("Initializing cellular ratio receiver...\n");
        if (CellularRatioInit()) {
            WriteOutput("Cellular ratio receiver initialized successfully\n");
            WriteOutput("Listening on: %s\n", CELLULAR_RATIO_SOCKET_PATH);
            if (CellularMode == 1) {
                g_CellularControlEnabled = 1;
                WriteOutput("Mode: LOG + CONTROL (ratio-based gain enabled)\n");
            } else {
                g_CellularControlEnabled = 0;
                WriteOutput("Mode: LOG ONLY (no algorithm change)\n");
            }
        } else {
            WriteOutput("Failed to initialize cellular ratio receiver\n");
        }
    }

    // Test mode: linear rate ramp
    uint32_t TestMode = 0;
    if (TryGetValue(argc, argv, "test", &TestMode) && TestMode == 1) {
        extern volatile int g_TestModeEnabled;
        g_TestModeEnabled = 1;
        WriteOutput("TEST MODE: Linear rate ramp 0 -> 50 Mbps over 60s\n");
    }
    ...
    if (UseTcp && EnableTcpLogging) {
        ...
        // 设置默认日志文件路径 - 确保目录存在
        int mkdirResult = system("mkdir -p /home/qwu26/msquic_cellular/artifacts/bbr_logs");
        if (mkdirResult != 0) {
        ...
        snprintf(logFilePath, sizeof(logFilePath), "/home/qwu26/msquic_cellular/artifacts/bbr_logs/tcp_bbr_%s.txt", timestamp);
    ...
    )
{
    ...
    // Clean up cellular ratio receiver
    if (EnableCellularRatio) {
        CellularRatioCleanup();
        WriteOutput("Cellular ratio receiver cleaned up\n");
    }

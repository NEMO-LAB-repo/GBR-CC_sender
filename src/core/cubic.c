...
//
// CUBIC log file - overwrite on first use, then append
//
static int g_CubicLogFileInitialized = 0;

//
// Cellular ratio weak symbols (same as BBR)
// 只声明，不定义！定义在 cellular_ratio.c 里
//
...
static inline int
IsCellularRatioAvailable_Cubic(void) {
    return g_CellularRatioAvailable != 0;
}

static inline double
GetCellularRatio_Cubic(void) {
    return IsCellularRatioAvailable_Cubic() ? g_CellularRatioValue : 1.0;
}
    ...
    )
{
    ...
    //
    // Log packet sent (CSV format for analysis)
    //
    {
        QUIC_CONNECTION* Connection = QuicCongestionControlGetConnection(Cc);
        const QUIC_PATH* Path = &Connection->Paths[0];
        uint64_t ConnectionDuration = CxPlatTimeUs64() - Connection->Stats.Timing.Start;
        uint64_t SmoothedRtt = Path->GotFirstRttSample ? Path->SmoothedRtt : 0;

        // Get cellular ratio info
        int CellularAvail = IsCellularRatioAvailable_Cubic();
        double ratio = GetCellularRatio_Cubic();
        int ratioScaled = (int)(ratio * 1000);  // Scale to integer (0-1000)

        // Estimate bandwidth as CWND / RTT
        uint64_t EstBW = (SmoothedRtt > 0) ? (Cubic->CongestionWindow * 1000000ULL / SmoothedRtt) : 0;
        uint64_t ActualBW = EstBW * 3 / 4;  // actualBw = estBw * 0.75

        FILE* logFile = fopen("/home/qwu26/msquic_cellular/artifacts/bbr_logs/cubic_log.txt",
                              g_CubicLogFileInitialized ? "a" : "w");  // First call overwrites, then appends
        if (logFile != NULL) {
            if (!g_CubicLogFileInitialized) {
                // Write header on first use
                fprintf(logFile, "# CUBIC Packet Log\n");
                fprintf(logFile, "# Format: timestamp_us,pkt_num,size,estBw,actualBw,cwnd,inflight,rtt,ssthresh,ratio_scaled,cellular_avail\n");
                g_CubicLogFileInitialized = 1;
            }

            // CSV: timestamp, pkt#, size, estBw, actualBw, cwnd, inflight, rtt, ssthresh, ratio, cellular_avail
            fprintf(logFile, "%lu,%lu,%u,%lu,%lu,%u,%u,%lu,%u,%d,%d\n",
                    (unsigned long)ConnectionDuration,
                    (unsigned long)(Connection->Stats.Send.TotalPackets),
